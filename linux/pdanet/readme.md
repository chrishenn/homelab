...
pdanet is an android app that will route mobile data through a phone's hosted wifi hotspot

---

## Linux: needs a client setup
...
proxying is per-protocol. possibly working on the http and https layer, but git uses some other protocol. probably
rewrite git protocol into http?

ping on icmp won't work because it's icmp protocol, but curl will work.
