#!/usr/bin/env python3
"""
https://github.com/steipete/homebrew-tap/blob/main/.github/scripts/generate_cask_api.py
...
def generate(
    ...
    # why would you ever do this, homebrew? why
    repo_parts = tap_repo.split("/")
    tap = "/".join([repo_parts[0], repo_parts[1].replace("homebrew-", "")])
    ...
    for cask_path in cask_paths:
        ...
        metadata = metadata_reader(tap_repo, token)
        validate_metadata(
            metadata,
            tap=tap,
            token=token,
...
def main() -> None:
    ...
    parser.add_argument("--tap", help="tap repo; eg: chrishenn/homebrew-tap")
    args = parser.parse_args()
    try:
        generate(tap_repo=args.tap)
